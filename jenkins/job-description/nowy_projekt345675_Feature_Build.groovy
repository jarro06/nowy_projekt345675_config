...
def job_config = [
    job: [
        name: "nowy_projekt345675_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
def customLib = library identifier: 'nowy_projekt345675_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/nowy_projekt345675_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.NowyProjekt345675.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "jarro06", "nowy_projekt345675", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("nowy_projekt345675_Feature_Build");
...
job.with {
    ...
    authenticationToken('lITSpdTcjPjrE1qCDRKoljCZ9050ZiXh')
