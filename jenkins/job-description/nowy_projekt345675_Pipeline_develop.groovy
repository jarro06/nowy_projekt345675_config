...
def job_config = [
        job: [
                name: "nowy_projekt345675_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "nowy_projekt345675", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("nowy_projekt345675_Pipeline_develop");
